...
/**
 * @param path 文件临时位置
 * @returns {null} null 或文件名
 */
util.generateFileKey = function(path) {
    path = path.trim();
    let md5 = require("md5.js");
    let fileKey = md5(path);
    let lastDot = path.lastIndexOf('.');
    if (path && lastDot) {
        let fileExt = path.substring(lastDot);
        fileKey = md5(path) + fileExt;
    }
...
// a: 对象, ajax 请求参数。 例如 url method dataType data success回调等
util.request = function (a) {
    ...
    if (-1 == i.indexOf("http://") && -1 == i.indexOf("https://") && (i = util.url(i)),
        getUrlParam(i, "state") || a.data && a.data.state || !n || (i = i + "&state=sid-" + n),
        !a.data || !a.data.m) {
            ...
            wx.login({
                success:(res)=>{
                    util.request({
                        ...
                        success: function(t) {
                            if (t.data.openid) {
                                wx.setStorageSync('hasOpenid', true);
